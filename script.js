/* ---------------- DATA ---------------- */
const PRODUCTS = [
  { id:'chilli', name:'Chilli Powder', bangla:'মরিচ গুঁড়া', tagline:'Blazing Red Heat', price:350, heat:8,
    accent:'#FF2D1A', accent2:'#7A0C0C', icon:'fa-pepper-hot', img:'images/chilli.jpg',
  ...
  { id:'cumin', name:'Cumin Powder', bangla:'জিরা গুঁড়া', tagline:'Earthy Warm Depth', price:320, heat:2,
    accent:'#C97C2B', accent2:'#5c3a12', icon:'fa-seedling', img:'images/cumin.jpg',
  ...
  { id:'turmeric', name:'Turmeric Powder', bangla:'হলুদ গুঁড়া', tagline:'Golden Glow Root', price:300, heat:0,
    accent:'#FFC700', accent2:'#a37c00', icon:'fa-sun', img:'images/turmeric.jpg',
    desc:'Pure golden turmeric root, sun-cured and finely milled. Earthy, slightly bitter-sweet and packed with color — essential for curries, lattes and everyday cooking.' },
  { id:'coriander', name:'Coriander Powder', bangla:'ধনিয়া গুঁড়া', tagline:'Citrusy Fresh Ground', price:280, heat:1,
    accent:'#7A8B3D', accent2:'#424d1e', icon:'fa-leaf', img:'images/coriander.jpg',
    desc:'Freshly ground coriander seeds with a bright, citrusy sweetness. The secret finishing touch for curries, chutneys, marinades and any dish that needs balance.' },
];

const SHOP_WHATSAPP = '8801XXXXXXXXX'; // <-- YOUR WhatsApp number (880 + number without leading 0)
const FREE_SHIP = 999;   // free delivery over ৳999
const DELIVERY = { inside:60, outside:120 }; // Dhaka / outside Dhaka

const REVIEWS = [
  { name:'Rahim U. — Dhaka', text:'Ordered the chilli powder for my restaurant. Real heat, deep colour — completely different from packet spices. Will reorder.', stars:5 },
  { name:'Sharmin A. — Chattogram', text:'The turmeric is so fresh and golden, you can smell the difference as soon as you open the jar. Delivery took 2 days.', stars:5 },
  { name:'Tanvir H. — Sylhet', text:'Cumin aroma is amazing. Cash on delivery worked smoothly, they called to confirm before sending. Trusted seller.', stars:5 },
  { name:'Nusrat J. — Khulna', text:'Took the full box of all four. Packaging was very secure, nothing broken. Coriander powder is my favourite so far.', stars:4 },
];
...
const FAQS = [
  ...
  { q:'Do you offer a discount for buying all spices?', a:'Yes! Use our Build-a-Box tool to mix quantities of any spices and get an automatic 15% bundle discount. You can also use code HOTSPY20 for 20% off.' },
  { q:'What are the delivery charges and time?', a:'Inside Dhaka: ৳60 • Outside Dhaka: ৳120 • FREE on orders over ৳999. Delivery takes 2–4 business days anywhere in Bangladesh via trusted couriers.' },
  { q:'How can I pay?', a:'Cash on Delivery is available nationwide. You can also pay via bKash or Nagad after we call you to confirm the order.' },
  { q:'What is your return policy?', a:'If a jar arrives damaged or you\'re not satisfied, message us on WhatsApp within 7 days with a photo and we\'ll replace it or refund you — no hassle.' },
];
...
const USAGE = {
  ...
  turmeric:{ title:'Turmeric Powder Missions', text:'Whisk into golden milk, stir into rice while cooking, or add to soups and stews for color and warmth — a daily kitchen essential.' },
  coriander:{ title:'Coriander Powder Missions', text:'Add early in cooking for curries and stews, or mix into chutneys, marinades and spice blends for a bright, citrusy sweetness.' }
};

let cart = JSON.parse(localStorage.getItem('hotspy_cart') || '[]');
let bundleQty = { chilli:1, cumin:1, turmeric:1, coriander:1 };
...
function jarHTML(p){
  if(p.img){
    return `<img src="${p.img}" alt="${p.name}" class="w-full h-auto object-contain" style="filter:drop-shadow(0 20px 25px rgba(0,0,0,.55))">`;
  }
  return `
...
function renderProducts(){
  ...
  grid.innerHTML = PRODUCTS.map(p=>`
    <div class="bg-coal border border-white/10 rounded-3xl p-7 flex flex-col items-center text-center hover:-translate-y-2 hover:shadow-glow transition-all">
      ...
      <p class="text-cream/50 text-xs mb-3 bangla">${p.bangla}</p>
      <div class="mb-4">${heatPips(p.heat,8)}</div>
      <p class="text-cream/60 text-sm mb-5">${p.desc}</p>
      <div class="flex items-center justify-between w-full mt-auto">
        <span class="font-display text-2xl text-gold">৳${p.price.toFixed(0)}</span>
...
function renderBundle(){
  ...
  grid.innerHTML = PRODUCTS.map(p=>`
    <div class="bg-coal border border-white/10 rounded-3xl p-6 flex flex-col items-center text-center">
      ...
      <p class="text-gold font-bold mb-3">৳${p.price.toFixed(0)}</p>
...
function updateBundleSummary(){
  ...
  PRODUCTS.forEach(p=>{
    ...
    if(qty>0){
      ...
      subtotal += line;
      rows += `<div class="flex justify-between text-cream/70"><span>${p.name} × ${qty}</span><span>৳${line.toFixed(0)}</span></div>`;
    }
  });
  summary.innerHTML = rows || `<p class="text-cream/40">Select spices to build your box.</p>`;
  const discount = subtotal * 0.15;
  const total = subtotal - discount;
  document.getElementById('bundleSub').textContent = '৳'+subtotal.toFixed(0);
  document.getElementById('bundleDisc').textContent = '-$'+discount.toFixed(0);
  document.getElementById('bundleTotal').textContent = '৳'+total.toFixed(0);
}
...
function renderCart(){
    ...
    box.innerHTML = cart.map(c=>{
      ...
      <div class="flex gap-4 items-center bg-ink/40 border border-white/10 rounded-2xl p-3">
        ...
        <div class="flex-1">
          ...
          <p class="text-cream/50 text-xs mb-2">৳${p.price.toFixed(0)} each</p>
        ...
        <div class="text-right">
          <p class="font-bold text-gold text-sm mb-2">৳${(p.price*c.qty).toFixed(0)}</p>
  ...
  document.getElementById('shipMsg').innerHTML = subtotal >= FREE_SHIP
    ? '🎉 FREE delivery unlocked!'
    : `Add <b>৳${(FREE_SHIP - subtotal).toFixed(0)}</b> more for FREE delivery`;
}
...
function submitOrder(){
  ...
  const subtotal = cart.reduce((a,c)=>{ const p = PRODUCTS.find(x=>x.id===c.id); return a + p.price*c.qty; },0);
  const delivery = subtotal >= FREE_SHIP ? 0 : DELIVERY[area];
  let msg = `🔥 HOTSPY ORDER%0A%0A👤 Name: ${encodeURIComponent(name)}%0A📞 Phone: ${encodeURIComponent(phone)}%0A🏠 Address: ${encodeURIComponent(address)}%0A🗺️ Area: ${area==='inside'?'Inside Dhaka':'Outside Dhaka'}%0A💳 Payment: ${encodeURIComponent(pay)}%0A%0A📦 Order:%0A`;
  cart.forEach(c=>{
    const p = PRODUCTS.find(x=>x.id===c.id);
    msg += `• ${encodeURIComponent(p.name)} x ${c.qty} = ৳${(p.price*c.qty).toFixed(0)}%0A`;
  });
  msg += `%0A🚚 Delivery: ৳${delivery}%0A💰 Total: ৳${subtotal + delivery}`;
...
function openQuickView(id){
  ...
  document.getElementById('qvPrice').textContent = '৳'+qvCurrent.price.toFixed(0);
...
function updateHeat(){
  ...
  else if(val>=3) rec = 'Cumin or Coriander Powder';
  ...
  document.getElementById('searchResults').innerHTML = results.map(p=>`
    <button onclick="jumpToProduct('${p.id}')" class="w-full flex items-center gap-4 text-left p-3 rounded-xl hover:bg-white/5 transition">
      ...
      <span>
        ...
        <span class="block text-cream/50 text-xs">৳${p.price.toFixed(0)} • ${p.tagline}</span>
      </span>
    </button>
